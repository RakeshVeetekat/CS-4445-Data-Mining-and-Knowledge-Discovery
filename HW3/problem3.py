...
def cosine_similarity(X, Y):
    ...
    S = np.dot(X,Y) / (math.sqrt(np.dot(X,X)) * math.sqrt(np.dot(Y,Y)))
...
def pairwise_item_sim(R):
    ...
    movies = len(R)
    users = len(R[0])
    S = np.zeros([movies,movies])

    for i in range(movies):
        for j in range(movies):
            # Compare movie i with other movies
            for m in range(movies):
                # Determine which users watched both movies
                user_ratings_movieA = []
                user_ratings_movieB = []
                for u in range(users):
                    if R[i,u] != None and R[m,u] != None:
                        user_ratings_movieA.append(R[i,u])
                        user_ratings_movieB.append(R[m,u])
                S[i,j] = cosine_similarity(user_ratings_movieA,user_ratings_movieB)
