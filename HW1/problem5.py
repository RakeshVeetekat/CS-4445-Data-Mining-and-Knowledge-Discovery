...
def compute_G(A, alpha = 0.95):
    ...
    #Need transition matrix and matrix filled with random transition probabilities to each node from any node
    S = compute_S(A)    #transition matrix

    num_of_nodes = np.shape(A)[0]
    rand_prob = 1.0/num_of_nodes
    random_transition_matrix = np.full((num_of_nodes,num_of_nodes),rand_prob)   #Matrix filled with random transition probabilities

    #Calculate matrix G using alpha
    G = alpha*S + (1-alpha)*random_transition_matrix
