...
def compute_P(A):
    ...
    vector_column_sums = np.sum(A,axis=0)
    array_transition_probabilities = np.diag(np.reciprocal(vector_column_sums))
    P = np.dot(A,array_transition_probabilities)
...
def random_walk(P, x_0, max_steps=10000):
    ...
    curr_pagerank_scores = x_0
    n_steps = 0

    #Start random walk
    for n in range(max_steps):
        #Update current pagerank scores after one random walk
        prev_pagerank_scores = curr_pagerank_scores
        curr_pagerank_scores = random_walk_one_step(P,prev_pagerank_scores)
        n_steps += 1

        #If probability vector stayed the same after random walk, final pagerank score can be extracted
        if np.allclose(prev_pagerank_scores,curr_pagerank_scores):
            break

    x = curr_pagerank_scores
