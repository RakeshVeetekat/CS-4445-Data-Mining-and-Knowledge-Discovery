...
class TicTacToe(BoardGame):
    ...
    def check_game(self,s):
        ...
        # check if either player won by row,column,diagonal
        e = 0

        # checking rows
        for i in range(0,3):
            if np.array_equal(s.b[i],[1,1,1]):
                e = 1
                return e
            elif np.array_equal(s.b[i],[-1,-1,-1]):
                e = -1
                return e

        # checking columns
        for j in range(0,3):
            if np.array_equal(s.b[:,j],[1,1,1]):
                e = 1
                return e
            elif np.array_equal(s.b[:,j],[-1,-1,-1]):
                e = -1
                return e

        # checking diagonals
        diag1 = s.b.diagonal()
        diag2 = np.fliplr(s.b).diagonal()
        for n in range(0,2):
            if np.array_equal(diag1,[1,1,1]):
                e = 1
                return e
            elif np.array_equal(diag1,[-1,-1,-1]):
                e = -1
                return e

            if np.array_equal(diag2,[1,1,1]):
                e = 1
                return e
            elif np.array_equal(diag2,[-1,-1,-1]):
                e = -1
                return e
        ...
        if len(list(zip(*np.where(s.b == 0)))) == 0:
            e = 0
            return e
        else:
            e = None
...
class RandomPlayer(Player):
    ...
    def choose_a_move(self,g,s):
        ...
        valid_moves = g.get_valid_moves(s=s)
...
class MMNode(Node):
    ...
    def expand(self,g):
       ...
       |-------------------------------
        ...
        move_state_pairs = np.asarray(g.get_move_state_pairs(s=self.s))

        # expand the node with one level of children nodes
        for m in range(0,len(move_state_pairs)):

            # for each next move m and game state s, create a child node
            Child = MMNode(s=move_state_pairs[m,1],p=self,m=move_state_pairs[m,0])
    ...
    def build_tree(self,g):
       ...
       |-------------------------------
        ...
        if g.check_game(s=self.s) is None:
...
class MiniMaxPlayer(Player):
    ...
    def choose_optimal_move(self,n):
        ...
        r,c = optimal_move[0],optimal_move[1]
    ...
    def choose_a_move(self,g,s):
        ...
        p = MiniMaxPlayer()
        r,c = p.choose_optimal_move(root)
