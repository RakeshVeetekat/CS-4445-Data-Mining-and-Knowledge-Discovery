...
class MCNode(Node):
    ...
    def sample(self,g):
       ...
       |-------------------
        ...
        playerX,playerO = RandomPlayer(),RandomPlayer()
        e = g.run_a_game(x_player=playerX,o_player=playerO,s=self.s)
    ...
    def expand(self,g):
       ...
       |-------------------------------
        ...
        for m in range(0,len(move_state_pairs)):
            ...
            Child = MCNode(s=move_state_pairs[m,1],p=self,m=move_state_pairs[m,0])
    ...
    def select_a_child(self):
        ...
        for child in self.c:
            child_ucb = child.compute_UCB(child.v,child.N,self.N,self.s.x)
