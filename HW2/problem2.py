...
def import_team_names(filename ='ncaa_teams.txt'):
    ...
    team_names = np.loadtxt(filename,dtype='str',delimiter=",")
...
def import_W(filename ='ncaa_results.csv'):
    ...
    W = np.loadtxt(filename,dtype='int',delimiter=",")
...
def team_rating(resultfile = 'ncaa_results.csv', 
    ...
    each_team_Elo = elo_rating(game_results,len(team_names),K)

    # sort team names according to their Elo ratings
    top_teams_with_ratings = sorted(zip(each_team_Elo,team_names),reverse=True)
    top_ratings, top_teams = zip(*top_teams_with_ratings)
