...
def test_compute_dL_da():
    ...
    np.seterr(all='raise')
    assert np.allclose(dL_da[0,0], 0., atol= 1e-3)
    assert dL_da[0,1] < -1e5
    assert dL_da[0,1] > -float('Inf')
...
def test_backward():
    ...
    assert da.shape == (1,2) 
    assert dz.shape == (2,2) 
    assert dW.shape == (2,2,3) 
    assert db.shape == (2,2) 

    da_true = np.mat('0.,-2.')
    dz_true = [[0.25, -0.25],[-0.25,0.25]]
    dW_true = [[[ 1., 2., 3.], [ 0., 0., 0.]],[[ 0., 0., 0.], [ 1., 2., 3.]]] 
    db_true = np.mat('1.,0.;0.,1.')
