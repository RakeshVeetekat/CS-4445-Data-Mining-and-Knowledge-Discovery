...
'''
    ...
    Notations:
            ...
            ---------- partial gradients ----------------------
            dL_da: the partial gradients of the loss function L w.r.t. the activations a, a float numpy matrix of shape 1 by c. 
            ...
            dz_dW: the partial gradient of logits z w.r.t. the weight matrix W, a numpy float matrix of shape (c by c by p). 
                   The (i,j,k)-th element of dz_dW represents the partial gradient of the i-th logit (z[i]) w.r.t. the weight W[j,k]:   d_z[i] / d_W[j,k]
            dz_db: the partial gradient of the logits z w.r.t. the biases b, a float matrix of shape c by c. 
                   Each element dz_db[i,j] represents the partial gradient of the i-th logit z[i] w.r.t. the j-th bias b[j]:  d_z[i] / d_b[j]
...
def compute_a(z):
    ...
    # Calculate each softmax activation
    for i in range(len(z)):
        if z[i,0]-max <= -10.:
            a.append(0.0)
        else:
            a.append(np.exp(z[i,0] - max)/sum)
...
def compute_dL_da(a, y):
    '''
        ...
        Input:
            ...
            y: the label of a training instance, an integer scalar value. The values can be 0,1,2, ..., or (c-1).
        Output:
            dL_da: the local gradients of the loss function w.r.t. the activations, a float numpy vector of shape 1 by c. 
    ...
    #########################################
    return np.transpose(np.asmatrix(dL_da))
...
def compute_dz_dW(x,c):
    '''
        ...
        Output:
            dz_dW: the partial gradient of logits z w.r.t. the weight matrix, a numpy float tensor of shape (c by c by p). 
                   The (i,j,k)-th element of dz_dW represents the partial gradient of the i-th logit (z[i]) w.r.t. the weight W[j,k]:   d_z[i] / d_W[j,k]
    '''
    #########################################
    ## INSERT YOUR CODE HERE

    dz_dW = np.zeros((c,c,len(x)))
    for i in range(c):
        for j in range(c):
...
def compute_dz_db(c):
    '''
        ...
        Input:
            c: the number of classes, an integer. 
        Output:
            dz_db: the partial gradient of the logits z w.r.t. the biases b, a float vector of shape c by c. 
                   Each element dz_db[i,i] represents the partial gradient of the i-th logit z[i] w.r.t. the j-th bias b[i]:  d_z[i] / d_b[j]
        Hint: you could solve this problem using 1 line of code.
    '''
    #########################################
    ## INSERT YOUR CODE HERE

    dz_db = np.diag(np.ones(c))

    #########################################
    return np.asmatrix(dz_db)
       ...
       Back Propagation: given an instance in the training data, compute the local gradients of the logits z, activations a, weights W and biases b on the instance. 
        ...
        Output:
            ...
            da_dz: the local gradient of the activation w.r.t. the logits z, a float numpy matrix of shape (c by c). 
                   The (i,j)-th element of da_dz represents the partial gradient ( d_a[i]  / d_z[j] )
            dz_dW: the partial gradient of logits z w.r.t. the weight matrix W, a numpy float array of shape (c by c by p). 
                   The i,j,k -th element of dz_dW represents the partial gradient of the i-th logit (z[i]) w.r.t. the weight W[j,k]:   d_z[i] / d_W[j,k]
            dz_db: the partial gradient of the logits z w.r.t. the biases b, a float vector of shape c by c. 
                   Each element dz_db[i,j] represents the partial gradient of the i-th logit z[i] w.r.t. the j-th bias b[j]:  d_z[i] / d_b[j]
    '''
       ...
       Given the local gradients, compute the gradient of the loss function L w.r.t. the logits z using chain rule.
        Input:
            ...
            da_dz: the local gradient of the activation w.r.t. the logits z, a float numpy matrix of shape (c by c). 
                   The (i,j)-th element of da_dz represents the partial gradient ( d_a[i]  / d_z[j] )
        Output:
            dL_dz: the gradient of the loss function L w.r.t. the logits z, a numpy float vector of shape 1 by c. 
                   The i-th element dL_dz[0,i] represents the partial gradient of the loss function L w.r.t. the i-th logit z[i]:  d_L / d_z[i].
    '''
    #########################################
    ## INSERT YOUR CODE HERE
    ...
    '''
       Given the local gradients, compute the gradient of the loss function L w.r.t. the weights W using chain rule. 
        Input:
            dL_dz: the gradient of the loss function L w.r.t. the logits z, a numpy float vector of shape 1 by c. 
                   The i-th element dL_dz[0,i] represents the partial gradient of the loss function L w.r.t. the i-th logit z[i]:  d_L / d_z[i].
            dz_dW: the partial gradient of logits z w.r.t. the weight matrix W, a numpy float tensor of shape (c by c by p). 
                   The i,j,k -th element of dz_dW represents the partial gradient of the i-th logit (z[i]) w.r.t. the weight W[j,k]:   d_z[i] / d_W[j,k]
        Output:
            dL_dW: the global gradient of the loss function w.r.t. the weight matrix, a numpy float matrix of shape (c by p). 
                   ...
                   The i,j-th element dL_dW[i,j] represents the partial gradient of the loss function L w.r.t. the i,j-th weight W[i,j]:  d_L / d_W[i,j]
        Hint: originally dL_dW was supposed to be of shape (1 by c by p), but in order to use it to update matrix W, we need to change the shape into (c by p)
        Hint: you could use np.tensordot() to solve this problem using 2 lines of code
    '''
    #########################################
    ## INSERT YOUR CODE HERE

    dL_dW = np.tensordot(dL_dz,dz_dW,axes=1)
    dL_dW = dL_dW[0,]
    ...
    '''
       Given the local gradients, compute the gradient of the loss function L w.r.t. the biases b using chain rule.
        Input:
            dL_dz: the gradient of the loss function L w.r.t. the logits z, a numpy float vector of shape 1 by c. 
                   The i-th element dL_dz[0,i] represents the partial gradient of the loss function L w.r.t. the i-th logit z[i]:  d_L / d_z[i].
            dz_db: the local gradient of the logits z w.r.t. the biases b, a float numpy matrix of shape c by c. 
                   The i-th element dz_db[i,j] represents the partial gradient ( d_z[i]  / d_b[j] )
        Output:
            dL_db: the global gradient of the loss function L w.r.t. the biases b, a float numpy vector of shape c by 1.
                   The i-th element dL_db[i] represents the partial gradient of the loss function w.r.t. the i-th bias:  d_L / d_b[i]
        Hint: originally dL_db was supposed to be of shape (1 by c), but in order to use it to update parameter b (c by 1), we need to change the shape into (c by 1)
        Hint: you could solve this problem using 1 line of code 
    '''
    #########################################
    ## INSERT YOUR CODE HERE

    dL_db = np.transpose(np.dot(dL_dz,dz_db))
...
def check_dL_da(a, y, delta=1e-7):
    ...
    dL_da = np.asmatrix(np.zeros((1,c))) # initialize the vector as all zeros
    for i in range(c):
        d = np.asmatrix(np.zeros((c,1)))
        d[i] = delta
        dL_da[0,i] = ( compute_L(a+d,y) 
                        - compute_L(a,y)) / delta
    return dL_da 
...
def check_dz_dW(x, W, b, delta=1e-7):
    '''
        ...
        Input:
            ...
            delta: a small number for gradient check, a float scalar.
        Output:
            dz_dW: the approximated local gradient of the logits w.r.t. the weight matrix computed by gradient checking, a numpy float matrix of shape (c by by c by p). 
                   The i,j,k -th element of dz_dW represents the partial gradient of the i-th logit (z[i]) w.r.t. the weight W[j,k]:   d_z[i] / d_W[j,k]
    '''
    ...
    for i in range(c):
        for j in range(c):
            for k in range(p):
                d = np.zeros((c,p))
                d[j,k] = delta
                dz_dW[i,j,k] = (compute_z(x,W+d, b)[i,0] - compute_z(x, W, b))[i,0] / delta
    return dz_dW
...
def check_dz_db(x, W, b, delta=1e-7):
    ...
    '''
    c,p = W.shape # number of classes and features
    dz_db = np.asmatrix(np.zeros((c,c)))
    for i in range(c):
        for j in range(c):
            d = np.asmatrix(np.zeros((c,c))) 
            d[j] = delta
            dz_db[i,j] = (compute_z(x,W, b+d)[i,0] - compute_z(x, W, b)[i,0]) / delta
    return dz_db
