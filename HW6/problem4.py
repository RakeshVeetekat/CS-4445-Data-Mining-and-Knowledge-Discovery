...
def compute_a1(z1):
    ...
    a1 = np.asmatrix(sr.compute_a(z1))
...
def compute_dL_da2(a2, y):
    ...
    dL_da2 = np.transpose(sr.compute_dL_da(a2,y))
...
def compute_da2_dz2(a2):
    ...
    da2_dz2 = np.transpose(sr.compute_da_dz(a2))
...
def compute_dz2_dW2(a1,c):
    ...
    dz2_dW2 = np.zeros((c,len(a1)))
    result = sr.compute_dz_dW(a1,c)
    for i in range(c):
        dz2_dW2[i,] = result[i,i,]

    #########################################
    return np.asmatrix(dz2_dW2)
...
def compute_da1_dz1(a1):
    ...
    da1_dz1 = np.zeros(len(a1))
    for i in range(len(a1)):
        da1_dz1[i] = a1[i]*(1-a1[i])

    #########################################
    return np.transpose(np.asmatrix(da1_dz1))
...
def compute_dz1_db1(h):
    ...
    dz1_db1 = np.transpose(np.asmatrix(np.ones([h])))
...
def compute_dL_dz1(dL_da1,da1_dz1):
    ...
    dL_dz1 = np.transpose(np.asmatrix(np.diagonal(np.dot(da1_dz1,dL_da1.T))))
...
def compute_gradients(dL_da2, da2_dz2, dz2_dW2, dz2_db2, dz2_da1, da1_dz1, dz1_dW1, dz1_db1):
    ...
    # the 2nd layer
    dL_dW2 = (dL_da2.T * da2_dz2)
    dL_dW2 = np.asmatrix(dL_dW2)
    dL_dW2 = dL_dW2 @ dz2_dW2

    dL_db2 = (dL_da2.T * da2_dz2)
    dL_db2 = np.asmatrix(dL_db2)
    dL_db2 = dL_db2 @ dz2_db2
    dL_db2 = dL_db2.T

    # the 1st layer
    dL_dW1 = dz1_dW1
    dL_dW1 = np.asmatrix(dL_dW1)
    dL_dW1 = dL_dW1 @ dL_da2.T

    dL_db1 = (dz2_da1 * da1_dz1)
    dL_db1 = np.asmatrix(dL_db1)
    dL_db1 = dL_db1 @ dz1_db1
