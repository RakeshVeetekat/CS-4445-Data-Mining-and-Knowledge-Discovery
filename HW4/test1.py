...
def test_compute_eigen_pairs():
    ...
    # a 3x3 matrix with three eigen pairs
    v1 = 10
    e1 = np.array([1.,1.,0.])
    e1 = e1/np.linalg.norm(e1)
    v2 = 6 
    ...
    X = v1*np.outer(e1,e1) + v2 * np.outer(e2,e2) + v3 * np.outer(e3,e3)
    ...
    for p in P:
        ...
        if np.allclose(v,v2):
            ...
            assert np.allclose(e,e1) or np.allclose(e,-e1)
            c+=1
        elif np.allclose(v,v3):
            assert np.allclose(e,e3) or np.allclose(e,-e3) 
            c+=1
        else:
            assert False  # incorrect eigen pair
    assert c ==3 # should return all 3 eigen pairs
