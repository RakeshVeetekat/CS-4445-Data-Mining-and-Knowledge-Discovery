...
def compute_D(A):
    ...
    degree_vec = [np.sum(n) for n in A]    # Get the degree for each node
    D = np.asarray(np.diag(degree_vec))    # Arrange each degree along the diagonal
...
def find_e2(L,tol= 1e-4):
    ...
    L_Ep = p1.compute_eigen_pairs(L)
    L_Ep = p1.sort_eigen_pairs(L_Ep,order='ascending')
    val,vec = map(list,zip(*L_Ep))
    e2 = vec[0]
