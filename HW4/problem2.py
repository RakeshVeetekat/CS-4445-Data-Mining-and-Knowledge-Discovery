...
def compute_C(Xc):
    ...
    e = np.ones((rows,1))
    X_prime = Xc - (np.dot(np.dot(e,e.transpose()),Xc))/rows
    Y = np.dot(X_prime.transpose(),X_prime)
    C = Y / (rows - 1)
...
def compute_P(C,k):
    ...
    Ep = p1.sort_eigen_pairs(Ep,order='descending')
    val,vec = map(list,zip(*Ep))
    P_all = []

    for n in range(len(val)):
        P_all.append(np.transpose(vec[n]))
